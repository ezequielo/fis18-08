...
app.post(API_BASE_URL + "/credits", (req, res) => {
    ...
    console.log(Date()+" - POST /credits");

    var credit = req.body;

    db.insert(credit);   //modificado

    res.sendStatus(201);
});

app.put(API_BASE_URL + "/credits", (req, res) => {
    // Forbidden
    console.log(Date()+" - PUT /credits");

    res.sendStatus(405);
});

app.delete(API_BASE_URL + "/credits", (req, res) => {
    // Remove all contacts
    console.log(Date()+" - DELETE /credits");
...
app.post(API_BASE_URL + "/credits/:project_id", (req, res) => {
    // Forbidden
    console.log(Date()+" - POST /credits");
...
app.get(API_BASE_URL + "/credits/:project_id", (req, res) => {
    // Get a single 
    var project_id = req.params.project_id;
    console.log(Date()+" - GET /credits/"+project_id);

    db.find({"project_id": project_id},(err,credits)=>{
        if(err){
            console.error("Error accesing DB");
            res.sendStatus(500);
        }else{
...
app.delete(API_BASE_URL + "/credits/:project_id", (req, res) => {
    // Delete a single contact
    var project_id = req.params.project_id;
    console.log(Date()+" - DELETE /credits/"+project_id);
...
app.put(API_BASE_URL + "/credits/:project_id", (req, res) => {
    // Update contact
    var project_id = req.params.project_id;
    var updatedCredit = req.body;
    console.log(Date()+" - PUT /credits/"+project_id);

    if(project_id != updatedCredit.project_id){
        res.sendStatus(409);
        return;
    }

    db.update({"project_id": project_id},updatedCredit,(err,numUpdated)=>{
        if(err){
            console.error("Error accesing DB");
            res.sendStatus(500);
        }else{
